...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['search'])) {
        $search = $_POST['search'];
        header("Location: ulasan.php?search=$search"); 
    } else {
        $id = $_POST['id_ulasan'];
        ...
        $update_query = "UPDATE tbl_ulasan 
                        SET isi_ulasan='$isi_ulasan', tgl_ulasan='$tgl_ulasan', id_user='$id_user' 
                        WHERE id_ulasan=$id";
...
if (isset($_GET['id'])) {
    ...
    $select_query = "SELECT * FROM tbl_ulasan WHERE id_ulasan=$id";
    $result = $conn->query($select_query);

    if ($result->num_rows == 1) {
        $row = $result->fetch_assoc();
        $id = $row["id_ulasan"];
        $isi_ulasan = $row["isi_ulasan"];
        $tgl_ulasan = $row["tgl_ulasan"];
        $id_user = $row["id_user"];
    } else {
        echo "Kosong";
        exit;
                    ...
                    <div class="sb-sidenav-menu">
                        <div class="nav">
                            <div class="sb-sidenav-menu-heading text-white">
                                ...
                                <a class="nav-link text-white fw-bolder" href="ulasan.php">
                                    <div class="sb-nav-link-icon"><i class="fa-sharp fa-light fa-comments text-white fw-bolder"></i></i></div>
                                    Ulasan
                                </a>
                        ...
                        <div class="row mt-3">
                            <div class="col">
                                <div class="card mb-4">
                                    ...
                                    <input type="hidden" name="id_ulasan" value="<?php echo $id; ?>">
                                        <div class="row">
                                            <div class="col-1">
                                                <label for="isi_ulasan">Isi Ulasan</label>
                                            </div>
                                            <div class="col-3">
                                                <textarea name="isi_ulasan" id="isi_ulasan" class="form-control" required><?php echo $isi_ulasan; ?></textarea><br><br>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-1">
                                                <label for="tgl_ulasan">Tanggal Ulasan</label>
                                            </div>
                                            <div class="col-3">
                                                <input type="date" name="tgl_ulasan" id="tgl_ulasan" class="form-control" value="<?php echo $tgl_ulasan; ?>" required><br><br>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-1">

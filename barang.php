...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['delete'])) {
        ...
        $delete_query = "DELETE FROM tbl_databarang WHERE id_barang = $id_to_delete";
        ...
        header("Location: updateBarang.php?id=$id_to_update"); 
    }
}
if (isset($_GET['search'])) {
    $search = $_GET['search'];
    $query = "SELECT * FROM tbl_databarang where id_barang LIKE '%$search%' OR nama LIKE '%$search%' OR deskripsi LIKE '%$search%' OR warna LIKE '%$search%' OR material LIKE '%$search%' OR tipe LIKE '%$search%'";
} else {
    $query = "SELECT * FROM tbl_databarang ORDER BY id_barang DESC";
}
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">
                            Data Barang
                            <a href="tambahBarang.php" class="btn btn-success ms-3">Tambah Barang</a>   
                        </h1>
                        <div class="row mt-3">
                            <div class="col">
                                <div class="card mb-4">
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?= $row["nama"] ?></td>
                                                    <td><?= $row["harga"] ?></td>
                                                    <td><img src="<?= $row["ft_barang"] ?>" alt="<?= $row["ft_barang"] ?>" style="max-width: 40%;max-height:40%;"></td>
                                                    <td><?= $row["ukuran"] ?></td>
                                                    <td><?= $row["deskripsi"] ?></td>
                                                    <td><?= $row["warna"] ?>, <?= $row["material"] ?>, <?= $row["tipe"] ?></td>
                                                    <td>
                                                        <div style="display: flex;">
                                                            <form method='post'>
                                                                <input type='hidden' name='update' value='<?= $row["id_barang"] ?>'>
                                                                <button type="submit" class="btn btn-outline-secondary me-2">
                                                                    <i class="fa-sharp fa-light fa-pen"></i>
                                                                </button>
                                                            </form>
                                                            <form method='post'>
                                                                <input type='hidden' name='delete' value='<?= $row["id_barang"] ?>'>

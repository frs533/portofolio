...
if($_POST){
    ...
    if (isset($_FILES["bukti_pembayaran"])) {
        ...
        $target_directory = "bukti/"; 
        $target_file = $target_directory . basename($_FILES["bukti_pembayaran"]["name"]);

        if (move_uploaded_file($_FILES["bukti_pembayaran"]["tmp_name"], $target_file)) {
    
            $update_transaksi = "UPDATE tbl_transaksi SET status_transaksi='Sudah', bukti_pembayaran='$target_file' WHERE id_transaksi=$id_transaksi";
            $update_pesanan = "UPDATE tbl_pesanan SET status_pesanan='berhasil',metode_pembayaran='$metode_pembayaran' WHERE id_user LIKE '$id_user' AND status_pesanan LIKE 'menunggu'";
        
            if ($conn->query($update_transaksi) === TRUE && $conn->query($update_pesanan) === TRUE) {
...
$id = $_SESSION['id'];
$query = "SELECT * FROM tbl_transaksi WHERE id_user LIKE '$id' AND status_transaksi LIKE 'Belum' ORDER BY id_transaksi DESC LIMIT 1";
                ...
                <div class="container-fluid my-5">
                    <div class="card mx-auto" style="width: 18rem;">
                        <div class="card-body">
                            ...
                            <?php 
                            if($data['metode_pembayaran']=="BRI"){
                                echo '<img src="assets/img/bri.png" class="img-fluid rounded-start mx-auto my-3 text-center" alt="...">';
                            } else {
                                echo '<img src="assets/img/ovo.png" class="img-fluid rounded-start mx-auto my-3" alt="...">';
                            }
                            ?>
                            <h6 class="card-subtitle text-muted mt-3">No. Rek/Virtual Account</h6>
                            <p class="card-text"><?= $data['no_rek']?></p>
                            <form action="" method="post" enctype="multipart/form-data">
                                <input type="text" name="id_transaksi" value="<?= $data['id_transaksi']?>" hidden>
                                <input type="text" name="metode_pembayaran" value="<?= $data['metode_pembayaran']?>" hidden>
                                <input type="file" name="bukti_pembayaran" class="form-control mt-3">
                                <button class="btn col-12 mt-3 text-white fw-bold" style="background-color: #C38B8C;" type="submit">Upload Bukti Pembayaran</button>
                            </form>
                        </div>
                    </div>
                </div>
